...
export function generateGrid(gridSize: GridSize): number[][] {
	let grid: number[][] = [];
...
export function generateRandomGrid(gridSize: GridSize, randPopDenc: number): number[][] {
	let grid: number[][] = [];
	for (let i = 0; i < gridSize.rows; i++) {
...
export function getNeighbours(grid: number[][], i: number, j: number): number {
	let numNeighbours = 0;
	neighbours.map((n) => {
		const realI = i + n[1];
		const realJ = j + n[0];
...
export function getNextGen(grid: number[][]): number[][] {
